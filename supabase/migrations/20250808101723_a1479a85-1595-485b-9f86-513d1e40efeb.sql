-- Fix critical parameter name collision in decrement_pack_sessions function
...
BEGIN
  ...
  -- Get current sessions_remaining with row lock (FIX: fully qualify table reference)
  SELECT sessions_remaining INTO current_remaining
  FROM public.session_packs sp
  WHERE sp.id = pack_id AND sp.trainer_id = decrement_pack_sessions.trainer_id
  FOR UPDATE;
  ...
  -- Atomically decrement sessions_remaining (FIX: fully qualify table reference)
